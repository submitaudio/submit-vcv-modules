...
// Dezelfde knop als Chain, uniform vergroot volgens de Orbit-components-SVG.
struct OrbitKnob : SvgKnob {
	OrbitKnob() {
		minAngle = -0.83 * M_PI;
		maxAngle = 0.83 * M_PI;
		setSvg(Svg::load(asset::plugin(pluginInstance, "res/OrbitKnob.svg")));
		shadow->opacity = 0.f;
	}
};
...
struct OrbitWidget : SubmitModuleWidget {
	...
	void appendContextMenu(Menu* menu) override {
		...
		menu->addChild(createMenuItem("Manual", "", []() {
			system::openBrowser("https://www.submitaudio.nl/vcv-rack-modules-metamodule-plugins/orbit/");
		}));
